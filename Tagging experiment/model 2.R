...
dexp <- aggregate(found~PIT+species+length.class+month+year+release.loca,data=tagging.exp,FUN = sum)
dexp$n <- aggregate(found~PIT+species+length.class+month+year+release.loca,data=tagging.exp,FUN = length)$found
...
par <- list(logitP_cod = rep(0,length(unique(dexp$length.class[dexp$species=="torsk"]))),
            ...
            logitTag.diff = 0,logitTime.diff = rep(0,7),logitLoca.diff=0,
            logitRefind.eff = 0)
...
nll <- function(par){
  ...
  for(i in 1:length(n)){
    ...
    if(species[i]=='torsk'){
      p[i] <- plogis(logitP_cod[c_sizes==length.class[i]]+diff.PIT+diff.time+diff.loca)
    }
    if(species[i]=='skrubbe'){
      p[i] <- plogis(logitP_flounder[f_sizes==length.class[i]]+diff.PIT+diff.time+diff.loca)
    }
  }
...
# 1. Number of estimated parameters (not counting random effects!)
k <- length(opt$par)
