...
sealFood <-  read.table("data/sealFood.csv",header=TRUE,sep=';')
...
Q <- data.frame(month=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug",
                        "Sep","Oct","Nov","Dec"),
                quarter=c(1,1,1,2,2,2,3,3,3,4,4,4))

seal.diet$quarter <- Q$quarter[match(seal.diet$Month,Q$month)] 
sdn1 <- aggregate(n~Year+quarter+ymp,data=seal.diet,FUN = mean)
sd <- aggregate(FW.cor~Year+quarter+Species,data=seal.diet,FUN = sum)
sd <- left_join(sd,aggregate(n~Year+quarter,data=sdn1,FUN = sum))
sd$yq <- paste(sd$Year,sd$quarter)

full.diet <- as.character((as.data.frame(table(sd$yq)) %>% filter(Freq==3))[,1])
sd <- sd %>% filter(yq %in% full.diet)


sealFood$year <- as.numeric(substr(sealFood$sampling,1,4))
sealFood$month <- substr(sealFood$sampling,6,8)
sealFood$quarter <- Q$quarter[match(sealFood$month,Q$month)]
sealFood$samp_count <- 1

sf <- aggregate(biomass~year+quarter+species,data=sealFood,FUN = sum)
sf$n_samp <- aggregate(samp_count~year+quarter+species,data=sealFood,FUN = sum)$samp_count

sf$yq <- paste(sf$year,sf$quarter)
sf <-  sf %>% filter(yq %in% full.diet)

meanS.B <- aggregate(biomass/n_samp~species,data=sf,FUN=mean)$biomass
meanS.B <- c(12136680.43,19171.99)
S.idx <- match(sf$species,prey)
sf$Bnorm <- (sf$biomass/sf$n_samp)/meanS.B[S.idx]


weights <- (aggregate(FW.cor~Species,data = sd,FUN=sum)$FW.cor[1:2])/
  sum(aggregate(FW.cor~Species,data = sd,FUN=sum)$FW.cor[1:2]) # for 1. cod, 2. flatfish and 3. herring

rm(list=setdiff(ls(),c('sd','sf','weights')))
names(sd) <- c("year","quarter","species","B","n","yq")
#####

# Prepare cormorant data
#####
cormFood <- read.table("data/cormorantFood.csv",header=TRUE,sep=';')
corm.diet <-  read.table("data/corm_diet.csv",header=TRUE,sep=';')
prey <- c("cod","flatfish")
...
corm.diet$quarter <- Q$quarter[match(corm.diet$month,Q$month)] 

cdn1 <- aggregate(n~year+quarter+dmyl,data=corm.diet,FUN = mean)
cd <- aggregate(B~year+quarter+species,data=corm.diet,FUN = sum)
cd <- left_join(cd,aggregate(n~year+quarter,data=cdn1,FUN = sum))
cd$yq <- paste(cd$year,cd$quarter)

full.diet <- as.character((as.data.frame(table(cd$yq)) %>% filter(Freq==3))[,1])
cd <- cd %>% filter(yq %in% full.diet)
cormFood$year <- cdn1$year[match(cormFood$sampling,cdn1$dmyl)]
cormFood$quarter <- cdn1$quarter[match(cormFood$sampling,cdn1$dmyl)]
cormFood$samp_count <- 1

cf <- aggregate(biomass~year+quarter+species,data=cormFood,FUN = sum)
cf$n_samp <- aggregate(samp_count~year+quarter+species,data=cormFood,FUN = sum)$samp_count

cf$yq <- paste(cf$year,cf$quarter)
cf <-  cf %>% filter(yq %in% full.diet)

meanC.B <- aggregate(biomass/n_samp~species,data=cf,FUN=mean)$biomass
meanC.B <- c(12136680.43,19171.99)

C.idx <- match(cf$species,prey)
cf$Bnorm <- (cf$biomass/cf$n_samp)/meanC.B[C.idx]
...
#####
cormFood <- read.table("data/cormorantFood_month.csv",header=TRUE,sep=';')
