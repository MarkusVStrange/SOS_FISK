...
vbgrHerring <- function(age,cohort){ # von Bertalanffy growth rate. Parameters from McQueen et al., 2019 with fixed hatching time
  ...
  u <- coefs$u # Amplitude of seasonality
  w <- coefs$w # Timing of peak growth
  t_hatch <- coefs$t_hatch_herring # time of hatching, Julian day / 365 - spawning from Jan. to July (a bit arbitrary from Hüssy et al., 2011), average set to peak spawning
  t <- age+t_hatch
  X_t <- t-floor(t) # time of year [0,1]
  phi_t <- u*sin(2*pi*(X_t-w))/(2*pi) # seasonal variability in growth
  phi_hatch <- u*sin(2*pi*(t_hatch-w))/(2*pi) # seasonal variability in growth
...
vbgrFlounder <- function(age,cohort){ # von Bertalanffy growth rate. Parameters from McQueen et al., 2019 with fixed hatching time
  ...
  u <- coefs$u # Amplitude of seasonality
  w <- coefs$w # Timing of peak growth
  t_hatch <- coefs$t_hatch_flounder # time of hatching, Julian day / 365 - spawning from Jan. to July (a bit arbitrary from Hüssy et al., 2011), average set to peak spawning
  t <- age+t_hatch
  X_t <- t-floor(t) # time of year [0,1]
  phi_t <- u*sin(2*pi*(X_t-w))/(2*pi) # seasonal variability in growth
  phi_hatch <- u*sin(2*pi*(t_hatch-w))/(2*pi) # seasonal variability in growth
...
vbgrPlaice <- function(age,cohort){ # von Bertalanffy growth rate. Parameters from McQueen et al., 2019 with fixed hatching time
  ...
  u <- coefs$u # Amplitude of seasonality
  w <- coefs$w # Timing of peak growth
  t_hatch <- coefs$t_hatch_plaice # time of hatching, Julian day / 365 - spawning from Jan. to July (a bit arbitrary from Hüssy et al., 2011), average set to peak spawning
  t <- age+t_hatch
  X_t <- t-floor(t) # time of year [0,1]
  phi_t <- u*sin(2*pi*(X_t-w))/(2*pi) # seasonal variability in growth
  phi_hatch <- u*sin(2*pi*(t_hatch-w))/(2*pi) # seasonal variability in growth
...
vbgrDab <- function(age,cohort){ # von Bertalanffy growth rate. Parameters from McQueen et al., 2019 with fixed hatching time
  ...
  u <- coefs$u # Amplitude of seasonality
  w <- coefs$w # Timing of peak growth
  t_hatch <- coefs$t_hatch_dab # time of hatching, Julian day / 365 - spawning from Jan. to July (a bit arbitrary from Hüssy et al., 2011), average set to peak spawning
  t <- age+t_hatch
  X_t <- t-floor(t) # time of year [0,1]
  phi_t <- u*sin(2*pi*(X_t-w))/(2*pi) # seasonal variability in growth
  phi_hatch <- u*sin(2*pi*(t_hatch-w))/(2*pi) # seasonal variability in growth
...
#####
prey <- c("cod","herring","flounder","plaice","dab")
samplings <- paste(rep(1991:2023,each=4),1:4)
n_samplings <- length(samplings)
n_prey <- length(prey)
...
for (i in 1:length(samplings)){
  ...
  herring.ages <- available.herring/rowSums(available.herring)
  ####
  ...
  flounder.av$B <- flounder.av$N*(flounder.coef$a*flounder.av$l^flounder.coef$b) # g
  flounder.av$avB <- pmax(0,seal_flatfish.pref(flounder.av$l*10))*flounder.av$B
  flounder.ages <- available.flounder/rowSums(available.flounder)
  ...
  plaice.av$B <- plaice.av$N*(plaice.coef$a*plaice.av$l^plaice.coef$b) # g
  plaice.av$avB <- pmax(0,seal_flatfish.pref(plaice.av$l*10))*plaice.av$B
  plaice.ages <- available.plaice/rowSums(available.plaice)
  ...
  dab.av$B <- dab.av$N*(dab.coef$a*dab.av$l^dab.coef$b) # g
  dab.av$avB <- pmax(0,seal_flatfish.pref(dab.av$l*10))*dab.av$B
  dab.ages <- available.dab/rowSums(available.dab)
...
write.table(sealFoodAges,"SealfoodAges_sim.csv",row.names = FALSE,sep=';')
write.table(sealFood,paste(data_wd,"Sealfood_sim_quarter.csv",sep=""),row.names = FALSE,sep=';')

#####
# simulate cormorant food index each quarter 1991-2023
#####
prey <- c("cod","herring","flounder","plaice","dab")
samplings <- paste(rep(1991:2023,each=4),1:4)
n_samplings <- length(samplings)
...
for (i in 1:length(samplings)){
  ...
  cormFoodAges$sampling[((i-1)*n_prey+1):(i*n_prey)] <- samplings[i]
  cormFoodAges$species[((i-1)*n_prey+1):(i*n_prey)] <- prey
  cormFoodAges[(i-1)*n_prey+1,3:10] <- colSums(cod.ages*cod.av$avB)/sum(colSums(cod.ages*cod.av$avB))
  cormFoodAges[(i-1)*n_prey+2,3:11] <- colSums(herring.ages*herring.av$avB)/sum(colSums(herring.ages*herring.av$avB))
  cormFoodAges[(i-1)*n_prey+3,3:13] <- colSums(flounder.ages*flounder.av$avB)/sum(colSums(flounder.ages*flounder.av$avB))
  cormFoodAges[(i-1)*n_prey+4,3:13] <- colSums(plaice.ages*plaice.av$avB)/sum(colSums(plaice.ages*plaice.av$avB))
  cormFoodAges[(i-1)*n_prey+5,3:13] <- colSums(dab.ages*dab.av$avB)/sum(colSums(dab.ages*dab.av$avB))
  
  print(i)
}

write.table(cormFoodAges,"CormorantfoodAges_sim.csv",row.names = FALSE,sep=';')
