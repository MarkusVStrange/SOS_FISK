...
codQ1 <- data.frame(Year=NA,Quarter=NA,Species="O",
                    Age=NA,cpue=NA)
ALK_Cod1 <- list()
for(i in 1:n_years){
  dat.i = attr(ALK[[i]], "data")
  n_hauls <- length(unique(dat.i$haul.id))
  alk.i <- matrix(NA,ncol=length(ages),nrow=n_hauls)
  for(j in 1:n_hauls){
    row=j
    alk.i[j,] = NageByHaul(row, ALK[[i]], returnALK = FALSE)
  }
  cpue.i <- colSums(alk.i)/sum(hh$HaulDur[which(hh$Year==years[i] & hh$Quarter==1)])*60 # CPUE: ind./hour

  c.i <- data.frame(Year=years[i],Quarter=1,Species="cod",
                    Age=ages,cpue=cpue.i)
...
#####
mc.cores<-1; library(parallel)
d<-subset(dAll, Species=="Gadus morhua",Quarter==4,ICES_SUB %in% 22:24)
#dAll<-NULL; gc(); ## lose dAll because it takes up a lot of memory
d<-addSpectrum(d,by=1)
## get idea about number of age groups to include
agetab<-xtabs(NoAtALK~Year+Age,data=d[[1]])
...
codQ4 <- data.frame(Year=NA,Quarter=NA,Species="O",
                    Age=NA,cpue=NA)
ALK_Cod1 <- list()
for(i in 1:n_years){
  dat.i = attr(ALK[[i]], "data")
  ...
  cpue.i <- colSums(alk.i)/sum(hh$HaulDur[which(hh$Year==years[i] & hh$Quarter==4)])*60 # CPUE: ind./hour
  
  c.i <- data.frame(Year=years[i],Quarter=4,Species="cod",
                    Age=ages,cpue=cpue.i)
...
# Flounder ALK Q1
#####
mc.cores<-1; library(parallel) # ,Year %in% 1993:2023
d<-subset(dAll, Species=="Platichthys flesus",Quarter==1,ICES_SUB %in% 22:24)
#dAll<-NULL; gc(); ## lose dAll because it takes up a lot of memory
d<-addSpectrum(d,by=1)
## get idea about number of age groups to include
agetab<-xtabs(NoAtALK~Year+Age,data=d[[1]])
...
flounderQ1 <- data.frame(Year=NA,Quarter=NA,Species="O",
                    Age=NA,cpue=NA)
ALK_flounder1 <- list()
for(i in 1:n_years){
  dat.i = attr(ALK[[i]], "data")
  ...
  n_hauls <- length(unique(dat.i$haul.id))
  alk.i <- array(NA,dim=c(length(cm.breaks)-1,length(ages),n_hauls)) # alk for year extrapolations
  NaA <- matrix(NA,ncol=length(ages),nrow=n_hauls) # numbers at age
  for(j in 1:n_hauls){
    row=j
    NaA[j,] = NageByHaul(row, ALK[[i]], returnALK = FALSE)
    
    if(i %in% 1:3) alk.i[,,j] = NageByHaul(row, ALK[[i]], returnALK = TRUE)
  }
  cpue.i <- colSums(NaA)/sum(hh$HaulDur[which(hh$Year==years[i] & hh$Quarter==1)])*60 # CPUE: ind./hour
  c.i <- data.frame(Year=years[i],Quarter=1,Species="flounder",
                    Age=ages,cpue=cpue.i)
  flounderQ1 <- rbind(flounderQ1,c.i)
  
  if(i %in% 1:3) ALK_flounder1[[years[i]]] <- apply(alk.i,c(1,2),FUN=mean)
  print(years[i])
}

dp <- subset(dAll, Species=="Platichthys flesus",Quarter==1,ICES_SUB %in% 22:24)
dp <-addSpectrum(dp,by=1)
hlp <- dp[["HL"]] %>% filter(!(Year %in% d$Year))
hhp <- dp[["HH"]] %>% filter(!(Year %in% d$Year))

ALK_93_95 <- (ALK_flounder1[["1993"]]+ALK_flounder1[["1994"]]+ALK_flounder1[["1995"]])/3
prevN <- aggregate(HLNoAtLngt~LngtCm+Year,data=hlp,FUN=sum)
prevN$Dur <- aggregate(HaulDur~Year,data = hhp,FUN=sum)$HaulDur
prevN$CPUE <- prevN$HLNoAtLngt/prevN$Dur*60# CPUE: ind./hour
...
#####
mc.cores<-1; library(parallel)
d<-subset(dAll, Species=="Platichthys flesus",Quarter==4,ICES_SUB %in% 22:24)
#dAll<-NULL; gc(); ## lose dAll because it takes up a lot of memory
d<-addSpectrum(d,by=1)
## get idea about number of age groups to include
agetab<-xtabs(NoAtALK~Year+Age,data=d[[1]])
...
flounderQ4 <- data.frame(Year=NA,Quarter=NA,Species="O",
                         Age=NA,cpue=NA)
ALK_flounder1 <- list()
for(i in 1:n_years){
  dat.i = attr(ALK[[i]], "data")
  ...
  c.i <- data.frame(Year=years[i],Quarter=4,Species="flounder",
                    Age=ages,cpue=cpue.i)
  flounderQ4 <- rbind(flounderQ4,c.i)
  print(years[i])
}
...
#####
mc.cores<-1; library(parallel)
d<-subset(dAll, Species=="Pleuronectes platessa",Year %in% factor(1995:2023),Quarter==1,ICES_SUB %in% 22:24)
#dAll<-NULL; gc(); ## lose dAll because it takes up a lot of memory
d<-addSpectrum(d,by=1)
## get idea about number of age groups to include
agetab<-xtabs(NoAtALK~Year+Age,data=d[[1]])
...
plaiceQ1 <- data.frame(Year=NA,Quarter=NA,Species="O",
                         Age=NA,cpue=NA)
ALK_plaice1 <- list()
for(i in 1:n_years){
  dat.i = attr(ALK[[i]], "data")
  ...
  c.i <- data.frame(Year=years[i],Quarter=1,Species="plaice",
                    Age=ages,cpue=cpue.i)
  plaiceQ1 <- rbind(plaiceQ1,c.i)
  print(years[i])
}

plaiceQ1 <- plaiceQ1[-1,]
rm(list=setdiff(ls(),c('dAll','codQ1','codQ4','herQ1','herQ4','plaiceQ1','plaiceQ4'
                       ,'plaiceQ1','plaiceQ4','dabQ1','dabQ4','WB_rects','hl_markus')))
#####
...
#####
mc.cores<-1; library(parallel)
d<-subset(dAll, Species=="Pleuronectes platessa",Year %in% factor(c(1994:1996,1999:2023)),Quarter==4,ICES_SUB %in% 22:24)
#dAll<-NULL; gc(); ## lose dAll because it takes up a lot of memory
d<-addSpectrum(d,by=1)
## get idea about number of age groups to include
agetab<-xtabs(NoAtALK~Year+Age,data=d[[1]])
...
plaiceQ4 <- data.frame(Year=NA,Quarter=NA,Species="O",
                       Age=NA,cpue=NA)
ALK_plaice1 <- list()
for(i in 1:n_years){
  dat.i = attr(ALK[[i]], "data")
  ...
  c.i <- data.frame(Year=years[i],Quarter=4,Species="plaice",
                    Age=ages,cpue=cpue.i)
  plaiceQ4 <- rbind(plaiceQ4,c.i)
  print(years[i])
}
...
herringQ1 <- data.frame(Year=NA,Quarter=NA,Species="O",
                    Age=NA,cpue=NA)
ALK_herring1 <- list()
for(i in 1:n_years){
  dat.i = attr(ALK[[i]], "data")
  ...
  c.i <- data.frame(Year=years[i],Quarter=1,Species="herring",
                    Age=ages,cpue=cpue.i)
  herringQ1 <- rbind(herringQ1,c.i)
  print(years[i])
}

herringQ1 <- herringQ1[-1,]
rm(list=setdiff(ls(),c('dAll','codQ1','codQ4','herQ1','herQ4','flounderQ1','flounderQ4'
                       ,'plaiceQ1','plaiceQ4','dabQ1','dabQ4','WB_rects','hl_markus')))
#####
