...
fitVBGR_ca <- function(dat, weights=c(1,1)){
  ...
  f <- function(par){
    ...
    t1 <- as.numeric(t1s[unique(Species)])-t_hatch
    t3 <- as.numeric(t3s[unique(Species)])-t_hatch
    #t2 <- (t1+t3)/2
    SD_m <- exp(logSD_m)
    SD_c <- exp(logSD_c)
    SD_age <- exp(logSDage)
    ...
    for(i in 1:length(cohorts)){
      ...
      for(j in 1:length(times.i)){
        ...
        wgts <- c((sum(mul[cohort.idx][age.idx])),sqrt(sum(mul[cohort.idx][age.idx])),1)[weights]
        
        weight <- mul[cohort.idx][age.idx]/wgts[2]*sd_fac
        
        weight_m <- sum(mul[cohort.idx][age.idx])/wgts[1]# or /sqrt(sum(mul[cohort.idx][age.idx]))
